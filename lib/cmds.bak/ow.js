...
exports.run = (client, msg, params = []) =>
  new Promise((resolve, reject) => {
    ...
    const tag = params[0]
      ? params.join(' ')
      : client.config.overwatch.default_battle_tag
    ...
    msg.channel
      .send(`Looking up **${plat}** OW stats for **${name}**. Please wait...`)
      .then((m) => {
        ...
        if (tag.includes('#')) {
          promiseAllUris.push(
            request.get(
              `https://owapi.net/api/v3/u/${tag.replace('#', '-')}/blob`,
            ),
          )
          platforms.push('pc')
        } else {
          promiseAllUris.push(
            request.get(`https://owapi.net/api/v3/u/${tag}/blob?platform=xbl`),
          )
          platforms.push('xbl')
          promiseAllUris.push(
            request.get(`https://owapi.net/api/v3/u/${tag}/blob?platform=psn`),
          )
          platforms.push('psn')
        }
        ...
        delayedPromiseAll(promiseAllUris)
          .then((results) => {
            ...
            for (let i = 0; i < results.length; i++) {
              if (results[i]) {
                ...
                regions.forEach((region) => {
                  let level
                  if (
                    platformStats[region] &&
                    platformStats[region].stats.quickplay
                  ) {
                    level =
                      platformStats[region].stats.quickplay.overall_stats
                        .level +
                      platformStats[region].stats.quickplay.overall_stats
                        .prestige *
                        100
            ...
            if (!highestLevel.level) {
              ...
              msg.channel
                .send(
                  `**${name}** has an Overwatch profile, but has no time played.`,
                )
                .then(resolve)
                .catch(reject)
              return
            ...
            const extractedData = scrapeData(
              client,
              highestLevel.platform[highestLevel.region],
              highestLevel.level,
            )
            const regionName =
              highestLevel.region === 'any' ? '' : `/${highestLevel.region}`
            extractedData.overwatchUri = `https://playoverwatch.com/en-us/career/${platform}${regionName}/${tag.replace(
              '#',
              '-',
            )}`
          ...
          .catch((err) => {
            m.delete().catch(client.logger.error)
            msg
              .reply(`There was an error retrieving OW stats.`)
...
function scrapeData(client, ow, level) {
  const rank = client.utils.get(
    ['stats', 'competitive', 'overall_stats', 'comprank'],
    ow,
  )
  const avatar = client.utils.get(
    ['stats', 'quickplay', 'overall_stats', 'avatar'],
    ow,
  )

  const qpTime = formatTime(
    client.utils.get(['stats', 'quickplay', 'game_stats', 'time_played'], ow),
  )
  const rankedTime = formatTime(
    client.utils.get(['stats', 'competitive', 'game_stats', 'time_played'], ow),
  )
  ...
  for (const hero in playtime.quickplay) {
    if (playtime.quickplay.hasOwnProperty(hero)) {
      qpHeroes.push({
        name: client.utils.capitalize(hero),
        hours: playtime.quickplay[hero],
      })
    }
  }
  ...
  for (const hero in playtime.competitive) {
    if (playtime.competitive.hasOwnProperty(hero)) {
      rankedHeroes.push({
        name: client.utils.capitalize(hero),
        hours: playtime.competitive[hero],
      })
    }
  }
  ...
  return {
    ...
    avatar,
    tier:
      client.utils.get(['stats', 'competitive', 'overall_stats', 'tier'], ow) ||
      '',
...
function buildStringToPost(client, stats) {
  ...
  if (stats.rank) {
    ...
    str += `Ranked: ${
      stats.rankedTime
    } played - Rank ${r} (${client.utils.capitalize(stats.tier)})\n`
    stats.rankedHeroes.forEach((hero) => {
      const padName = padString(hero.name, 9)
      const timePlayed = padTime(hero.hours)
      str += `${
        stats.rankedHeroes.indexOf(hero) + 1
      }: ${padName} - ${timePlayed}\n`
    })
  }
  str = `\`\`\`qml\n${str}\`\`\``
