...
exports.run = (client, msg, params = []) =>
  new Promise((resolve, reject) => {
    ...
    if (numberOnly.test(query)) {
      embed.addField(
        `1d${query}`,
        `**${client.utils.getRandomIntInclusive(1, parseInt(query))}**`,
      )
    ...
    if (dice.length === 0 || !dice[0].text) {
      return msg.channel.send('Invalid input.').then(resolve).catch(reject)
    }

    if (dice.length < 2 && dice[0].text.split('d')[0] === '1') {
      embed.addField(
        query,
        `**${client.utils.getRandomIntInclusive(
          1,
          parseInt(dice[0].text.split('d')[1]),
        )}**`,
      )
    ...
    fs.readFile(
      path.join(__dirname, '../../assets/rolltable.png'),
      { encoding: null },
      (err, data) => {
