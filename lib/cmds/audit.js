  ...
  new Promise(async (resolve, reject) => {
    ...
    if (params[0] === 'list') {
      ...
      try {
        ...
        client.logger.error(
          'Error getting audit channel data from mongoDB',
          err,
        )
        reject(err)
        return
      ...
      if (owner && params[1] === 'all') {
        ...
        if (names.length === 0) {
          msg.channel
            .send('No channels currently post audit messages.')
            .then(resolve)
            .catch(reject)
          return
        }
      ...
      if (names.length === 0) {
        msg.channel
          .send('No channels on this server currently post audit messages.')
          .then(resolve)
          .catch(reject)
        return
      }
      ...
      let channel
      try {
        channel = await client.mongo.auditChannels.findOne({
  ...
  return new Promise((resolve, reject) => {
    if (!channel) {
      ...
      entry
        ...
        .then(() => {
          msg.channel
            .send('This channel will now post audit messages.')
            .then(resolve)
            .catch(reject)
        })
        .catch((err) => {
      ...
      msg.channel
        .send('This channel already posts audit messages.')
        .then(resolve)
        .catch(reject)
    }
  })
...
function removeChannel(client, msg, channel) {
  return new Promise(async (resolve, reject) => {
    if (!channel) {
      msg.channel
        .send("This channel doesn't currently post audit messages.")
        .then(resolve)
        .catch(reject)
      return
    }
    ...
    msg.channel
      .send('This channel will no longer post audit messages.')
      .then(resolve)
      .catch(reject)
  })
}
