...
exports.info = {
  ...
  usage: '[join | leave | set | done | notify | history | delete]',
  aliases: [],
};
...
exports.run = (client, msg, params = []) => new Promise((resolve, reject) => {
  ...
  if (params[0] === 'notify') {
    if (pres()) {
      ...
      const str = msg.content.replace(`${msg.prefix}bc notify`, '').trim();
      bcRole.setMentionable(true)
        .then(() => msg.delete())
        .then(() => msg.channel.send(`<@&${bcRole.id}>\n${str}`))
  ...
  // Book Club President wants a list of all the bc members
  if (params[0] === 'list') {
    if (pres()) {
      // Get the list of BC members
      const list = msg.guild.members.filter(m => m.roles.has(bcRole.id));
      const names = list.map(m => m.user.tag);
      if (!names || names.length === 0) {
        msg.author.send('There are no Book Club members.').then(resolve).catch(reject);
        return;
      }
      msg.author.send(names, { code: 'text' }).then(resolve).catch(reject);
    }
    return;
  }

  // Book Club President want to set the current book
  if (params[0] === 'set') {
    if (pres()) {
  ...
  // Book Club President want to delete a db entry
  if (params[0] === 'delete') {
    if (pres()) {
      const num = parseInt(params[1]);
      if (isNaN(num)) {
        msg.reply('Please enter a number of the book entry you would like to delete from the History list.')
          .then(resolve.catch(reject));
        return;
      }
      client.mongo.bookClub.find()
        .then(results => {
          if (num < 1 || num > results.length) {
            msg.reply('That is not an available book entry position.').then(resolve).catch(reject);
            return null;
          }
          const sorted = results.sort((a, b) => b.date_added - a.date_added);
          return sorted[num - 1].remove();
        })
        .then(() => {
          msg.reply(`Entry position ${num} removed successfully`).then(resolve).catch(reject);
        })
        .catch(reject);
    }
    return;
