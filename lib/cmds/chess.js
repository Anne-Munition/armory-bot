...
// Save botID from discord when discord is ready
  ...
  checkConditions(channel, chessData, (str, exit) => {
    ...
    if (exit) {
      channel.sendMessage(`Game Over.\nYou have 2 minutes to save the ` +
        `\`\`${prefix}chess <moves | fen | pgn | board>\`\`, then this game will be auto deleted.`);
      setTimeout(() => {
        removeGame(channel);
        channel.sendMessage('Chess game deleted');
      }, 1000 * 60 * 2);
      return;
    }
    str = null;
