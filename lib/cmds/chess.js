...
exports.run = (discord, msg, params = []) => {
  botID = discord.client.user.id;
  // Post usage if no parameters were passed
  if (params.length === 0) {
    msg.channel.sendMessage(`\`\`${prefix}chess <new | delete | info | moves | board | ascii | fen | pgn>\`\``);
    return;
  }
  ...
  switch (cmd) {
    ...
    case 'info':
      showInfo(discord, msg, chessData);
      return;
    // Show moves history
    case 'moves':
    ...
    case 'new':
      createGame(discord, msg, chessData, params);
      return;
    default:
      break;
  }
  ...
  switch (cmd) {
    ...
    case 'confirm':
      confirmDelete(msg, chessData);
      return;
    // Undo a chess move if timer allows
    case 'undo':
      undo(discord, msg, chessData);
      return;
    default:
      break;
  }
  ...
  playerGo(discord, msg, chessData, params);
};
...
function showInfo(discord, msg, chessData) {
  ...
  if (chessData) {
    ...
    str += `History: '${prefix}chess moves' to see move history`;
...
function createGame(discord, msg, chessData, params) {
  ...
  msg.channel.sendMessage('Creating new Chess game...')
    .then(m => {
      ...
      firstMove(discord, msg, chessGames[msg.channel.id]);
    })
    .catch(err => {
      logger.error(err);
    });
...
function deleteGame(msg, chessData) {
  // Show how to make a game  if one  doesn't exist yet
  if (!chessData) {
    createGameMessage(msg);
    return;
  }
...
function confirmDelete(msg, chessData) {
...
function showBoard(msg, chessData) {
  // Create the board
  createChessBoardImage(chessData.game)
    ...
    .catch(err => {
      ...
      msg.channel.sendMessage('There was an error processing the chessboard image. Please try again.')
        .then(m => {
...
function undo(discord, msg, chessData) {
  ...
  processMove(discord, msg, chessData);
}
...
function composite(image, data) {
  return new Promise((resolve, reject) => {
    ...
    const x = ((data.r - 1) * 45) + 5;
    const y = ((8 - data.f) * 45) + 4;
    gm(image)
...
function firstMove(discord, msg, chessData) {
...
function playerGo(discord, msg, chessData, params) {
  ...
  if (successfulMove) {
    ...
    processMove(discord, msg, chessData, params[0], true);
  } else {
...
function aiGo(discord, msg, chessData) {
  logger.debug('Chess AI go');
  if (chessData.ai_type === 1) {
    // AI type 1 = random Math moves
    const move = getRandomMove(chessData);
    // Delay a second so you can read the bot massage before it auto deletes
    logger.debug('random ai chess move type 1', move);
    setTimeout(() => {
      // Move the move we moved
      chessData.game.move(move);
      processMove(discord, msg, chessData, move);
    }, 1000);
  } else if (chessData.ai_type === 2) {
    // AI type 2 = AI moves
    // The response should have returned by now, if not we will use a random move
    if (chessData.ai_response === null) {
      // Use a random move
      const move = getRandomMove(chessData);
      logger.debug('random ai chess move type 2', move);
      setTimeout(() => {
        // Move the move we moved
        chessData.game.move(move);
        processMove(discord, msg, chessData, move);
...
function processMove(discord, msg, chessData, move, player) {
            ...
            msg.channel.sendMessage(str)
              .then(n => {
                ...
                chessData.timerUndo = setTimeout(() => {
                  ...
                  afterUndoExpired(discord, msg, chessData, n);
                }, 1000 * undoSeconds);
              })
              .catch(logger.error);
          } else {
            // This was a bot that moved; post next player info
            afterUndoExpired(discord, msg, chessData);
          }
        })
        .catch(logger.error);
    })
...
function afterUndoExpired(discord, msg, chessData, undoMsg) {
  ...
  if (chessData[chessData.game.turn()] === botID) {
    ...
    let user = chessData[chessData.game.turn()];
    if (user !== 'anyone') {
      // If the user is not set to anyone get their user object
      if (msg.channel.type === 'dm') {
        user = discord.client.users.find('id', user);
      } else {
        user = msg.guild.members.find('id', user);
      }
    }
    // prepend an @ onto anybody - it will not ping anybody
    if (user === 'anyone') {
      user = '@anyone';
    }
