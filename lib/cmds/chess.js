'use strict';
exports.info = {
  desc: 'Play a game of chess against another player or AI.',
  usage: '<commands>',
  aliases: [],
};
...
exports.run = (client, msg, params = []) => {
 /* botID = client.user.id;
  ...
  if (params.length === 0) {
    msg.channel.sendMessage(`\`\`${msg.prefix}chess ` +
      `<new | delete | undo | info | moves | board | ascii | fen | pgn>\`\``);
    return;
  }
  // Get any game data for this channel if it exists
  const chessData = client.chessGames.get(msg.channel.id);
  ...
  if (!isAPlayer(msg, chessData) && msg.author.id !== '84770528526602240') {
    msg.reply(`you are not a part of this game. \`\`${msg.prefix}chess info\`\``)
      .then(m => {
        m.delete(3000);
        msg.delete(3000);
      })
  ...
  if (msg.author.id !== chessData[chessData.game.turn()] && chessData[chessData.game.turn()] !== 'anyone') {
...
function showInfo(msg, chessData) {
  logger.debug('chess info');
  // Get player names
  const white = resolvePlayer(msg.channel, chessData.w);
  const black = resolvePlayer(msg.channel, chessData.b);
  // Who's turn is it?
  ...
  const block = `\`\`\`qml\n${str}\`\`\`\`\`${msg.prefix}chess moves\`\` to see move history.`;
  msg.channel.sendMessage(block);
}

function createGameMessage(msg) {
  msg.channel.sendMessage('There are no chess games running in this channel. To make a new game run:\n' +
    `\`\`${msg.prefix}chess new <whitePlayer> <blackPlayer>\`\` using an @ mention, 'me', 'bot' or 'any' ` +
    `for the player.`);
}

function createGame(client, msg, chessData, params) {
  // Exit if game data exists
  if (chessData) {
    msg.channel.sendMessage(`There is already a game running in this channel.\n` +
      `\`\`${msg.prefix}chess info\`\` or \`\`${msg.prefix}chess delete\`\``);
    return;
  }
  ...
  if (params.length < 3) {
    msg.channel.sendMessage('Please provide the names of both players. ' +
      ...
      `\`\`${msg.prefix}chess new <whitePlayer> <blackPlayer>\`\``);
    return;
  }
  ...
  msg.channel.sendMessage('Creating new Chess game...')
    .then(m => {
      ...
      client.chessGames.set(msg.channel.id, {
        game: new Chess(),
        w: white,
        b: black,
      ...
      saveGame(client, msg.channel.id)
        .then(() => {
          m.edit('Game created successfully. :thumbsup:');
          firstMove(msg.channel, client.chessGames.get(msg.channel.id));
        })
        .catch(err => {
          logger.error('Error saving create chess to mongoDB', err);
          m.edit('Error creating game. :thumbsdown:');
        });
    })
    .catch(err => {
      logger.error(err);
...
function deleteGame(msg, chessData) {
  logger.debug('need confirmation to make new game');
  // Tell player how to confirm deletion
  msg.channel.sendMessage(`Run: \`\`${msg.prefix}chess confirm\`\` within 10 seconds to confirm deletion.\n` +
    `Run: \`\`${msg.prefix}chess info\`\` for current game info.`);
...
function saveGame(client, id) {
  return new Promise((resolve, reject) => {
    const data = client.chessGames.get(id);
    data.channel_id = id;
    const entry = client.mongo.chessGames(data);
    entry.save()
      .then(resolve)
      .catch(reject);
  });
...
function processMove(msg, chessData, move, player) {
  ...
  if (move) {
    ...
    if (!player) {
      msg.channel.sendMessage(`My move was \`\`${move}\`\``);
    }
  }
        ...
        .then(m => {
          ...
          if (player) {
            const str = `You have ${undoSeconds} seconds for to run ` +
              `\`\`${msg.prefix}chess undo\`\` if you made a mistake.`;
            ...
            msg.channel.sendMessage(str)
              .then(n => {
                ...
                chessData.timerUndo = setTimeout(() => {
                  ...
                  afterUndoExpired(msg.channel, chessData, n);
                }, 1000 * undoSeconds);
              })
              .catch(logger.error);
          } else {
            // This was a bot that moved; post next player info
            afterUndoExpired(msg.channel, chessData);
          }
        })
        .catch(logger.error);
    })
...
function afterUndoExpired(msg, chessData, undoMsg) {
  ...
  checkConditions(msg, chessData, (str, exit) => {
    if (str) {
      msg.channel.sendMessage(str);
    }
    if (exit) {
      msg.channel.sendMessage(`Game Over.\nYou have 2 minutes to save the ` +
        `\`\`${msg.prefix}chess <moves | fen | pgn | board>\`\`, then this game will be auto deleted.`);
      setTimeout(() => {
        removeGame(msg.channel);
        msg.channel.sendMessage('Chess game deleted');
      }, 1000 * 60 * 2);
      return;
    }
    str = null;
...
function checkConditions(msg, chessData, callback) {
  // Exiting conditions
  if (chessData.game.in_checkmate()) {
    let matee = chessData.game.turn();
    let mater = matee === 'b' ? 'w' : 'b';
    matee = resolvePlayer(msg.channel, chessData[matee], true);
    mater = resolvePlayer(msg.channel, chessData[mater], true);
