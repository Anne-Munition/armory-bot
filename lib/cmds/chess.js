...
function showInfo(channel, chessData) {
  ...
  if (chessData) {
    ...
    if (chessData.w === 'anyone') {
      ...
      white = channel.client.users.find('id', chessData.w).username;  //TODO
    } else {
      white = channel.guild.members.find('id', chessData.w);
      if (white) {
    ...
    if (chessData.b === 'anyone') {
      ...
      black = channel.client.users.find('id', chessData.b).username; //TODO
    } else {
      black = channel.guild.members.find('id', chessData.b);
      if (black) {
    ...
    channel.sendCode('qml', utils.clean(str));
  } else {
...
function composite(image, data) {
  return new Promise((resolve, reject) => {
    ...
    const x = ((data.r - 1) * 26) + 0;
    const y = ((8 - data.f) * 26) + 0;
    gm(image)
...
function afterUndoExpired(channel, chessData, undoMsg) {
  ...
  if (chessData[chessData.game.turn()] === botID) {
    ...
    if (user !== 'anyone') {
      ...
      if (channel.type === 'dm') {
        user = channel.client.users.find('id', user); //TODO
      } else {
        user = channel.guild.members.find('id', user);
      }
    }
...
function checkConditions(msg, chessData) {
  if (chessData.chess.game_over()) {
    msg.channel.sendMessage('The game is over.');
    return;
  }
  if (chessData.chess.in_checkmate()) {
    msg.channel.sendMessage(`You have been mated in ${chessData.chess.history().length} moves. :sad:`);
    return;
  }
  if (chessData.chess.in_check()) {
    msg.channel.sendMessage(`You are in check.`);
    return;
  }
  if (chessData.chess.in_stalemate()) {
    msg.channel.sendMessage(`You have been stalemates in ${chessData.chess.history().length} moves.`);
    return;
  }
  if (chessData.chess.in_draw()) {
    const reason = chessData.chess.insufficient_material() ? 'Insufficient Materials' : '50-move rule';
    msg.channel.sendMessage(`This game has ended in a draw. Reason: ${reason}`);
    return;
  }
  if (chessData.chess.in_threefold_repetition()) {
