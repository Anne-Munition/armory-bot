...
exports.run = (client, msg) => new Promise((resolve, reject) => {
  ...
  // Post list to user's dm mchannel
  client.utils.getDmChannel(msg).send(commands.sort().join('\n'), { code: 'apache' })
    .then(() => {
    ...
    .catch(() => msg.reply('Unable to send you a list of commands, Did you block me?'));
});
