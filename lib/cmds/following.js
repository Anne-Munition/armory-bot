...
exports.run = (client, msg, params = []) => new Promise(async(resolve, reject) => {
  ...
  try {
    ...
    if (!result.users.filter(x => x.name === user.toLowerCase())) {
      msg.reply(`**${user}** is not a Twitch channel.`).then(resolve).catch(reject);
      return;
    } else if (!result.users.filter(x => x.name === channel.toLowerCase())) {
      msg.reply(`**${channel}** is not a Twitch channel.`).then(resolve).catch(reject);
      return;
    } else {
      user = {
