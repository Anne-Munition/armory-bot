...
exports.run = (client, msg, params = []) => new Promise((resolve, reject) => {
  ...
  msg.channel.send(`Searching Giphy for: **${query}**`)
    .then(async message => {
      // Create search uri
      ...
      request.get(uri).then(results => {
        ...
        if (filtered.length === 0) {
          message.edit(`No Giphy results found for \`\`${query}\`\` after filtering for size and content.`)
        ...
        .catch(err => {
          message.edit(`Error getting results from Giphy. **${err.message}**`).then(resolve).catch(reject);
        });
    })
