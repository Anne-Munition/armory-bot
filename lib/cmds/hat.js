...
exports.run = (client, msg, params = []) =>
  new Promise(async (resolve, reject) => {
    ...
    const avatarUri = msg.author.displayAvatarURL()
    // Exit if no avatar. This should not happen
    if (!avatarUri) {
      msg
        .reply('Unable to get your avatar. Cannot continue the operation')
        .then(resolve)
        .catch(reject)
      return
    }
    ...
    if (!translateParams) {
      msg.author
        .send(
          `Format error. Type \`\`${msg.prefix}hat\`\` to see usage options.`,
        )
        .then(resolve)
        .catch(reject)
      return
    ...
    fs.exists(hatFile, (exists) => {
      ...
      msg.author
        ...
        .then((m) => {
          processHat(hatData)
            .then((data) => {
              postHat(client, data, m, msg, params[0])
                .then(resolve)
                .catch(reject)
            })
            .catch((err) => {
              ...
              msg.edit(
                `Error processing your '${params[0]}' hat. Please try again...`,
              )
              reject(err)
            })
        })
    ...
    msg.author
      ...
      .then(() => {
        fs.unlink(data.tempHatPath, (err) => {
          if (err) {
            data.client.logger.error(
              'error un-linking file',
              data.tempHatPath,
              err,
            )
          }
...
function getHatSize(hatData) {
  return new Promise((resolve, reject) => {
    gm(hatData.hatBuffer).size((err, hatSize) => {
      if (err) {
        reject(err)
      } else {
...
function downloadAvatar(hatData) {
  return new Promise((resolve, reject) => {
    request
      .get(hatData.avatarUri, { encoding: null })
      .then((response) => {
        hatData.avatarBuffer = response.body
        resolve(hatData)
      })
      .catch(reject)
  })
...
function getModifiedHatSize(hatData) {
  return new Promise((resolve, reject) => {
    gm(hatData.hatBufferEdited).size((err, hatModifiedSize) => {
      if (err) {
        reject(err)
      } else {
...
function saveModifiedHat(hatData) {
  return new Promise((resolve, reject) => {
    const tempHatPath = path.join(
      hatData.client.tempDir,
      `hat_${hatData.authorId}_${hatData.hat}.png`,
    )
...
function applyHatComposite(hatData) {
  return new Promise((resolve, reject) => {
    const x =
      hatData.endX >= 0
        ? `+${hatData.endX.toString()}`
        : hatData.endX.toString()
    const y =
      hatData.endY >= 0
        ? `+${hatData.endY.toString()}`
        : hatData.endY.toString()
    gm(hatData.avatarBuffer)
    ...
    msg.author
      .send(
        `\`\`\`Examples:\n${msg.prefix}hat <name> ` +
          ...
          `Hats: xmas | hallow | pilgrim\`\`\`I cannot set your avatar via the bot, so you will have to save it yourself manually.`,
      )
      .then(resolve)
      .catch(reject)
  })
