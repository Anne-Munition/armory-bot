...
exports.run = (client, msg, params = []) => new Promise(async(resolve, reject) => {
  ...
  if (msg.channel.type === 'text') {
    ...
    sendHatFormatMessage(msg).then(resolve).catch(reject);
    return;
  }
  ...
  fs.exists(hatFile, (exists) => {
    ...
    if (!exists) {
      msg.author.sendMessage(`'${params[0]}' is not a registered hat.`).then(resolve).catch(reject);
      return;
    }
    const hatData = {
      client,
    ...
    msg.author.sendMessage('Processing...')
      .then(m => {
        processHat(hatData)
          .then(data => {
            postHat(data, m, msg, params[0]).then(resolve).catch(reject);
          })
          .catch(err => {
            logger.error(`Error processing '${params[0]}' hat`);
    ...
    msg.author.sendFile(data.avatarBufferEdited, `${msg.author.id}_${name}.png`)
      .then(message => {
        fs.unlink(data.tempHatPath, err => {
          if (err) {
            logger.error('error un-linking file', data.tempHatPath, err);
          }
        });
        // console.log(message.attachments.first().url);
        // TODO: some way for the user to authorize the bot to change the avatar for them automatically
      }).catch(reject);
  });
}
...
function downloadAvatar(hatData) {
  return new Promise((resolve, reject) => {
    utils.requestBuffer(hatData.avatarUri)
      .then(buffer => {
...
function sendHatFormatMessage(msg) {
  return new Promise((resolve, reject) => {
    msg.author.sendMessage(`\`\`\`Examples:\n${config.prefix}hat <name> ` +
      ...
      `Hats: xmas | hallo\`\`\``).then(resolve).catch(reject);
  });
}
