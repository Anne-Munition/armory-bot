...
exports.run = (client, msg, params = []) => new Promise(async (resolve, reject) => {
  ...
  if (!avatarUri) {
    msg.reply('Unable to get your avatar. Cannot continue the operation').then(resolve).catch(reject);
    return;
  }
  ...
  if (!translateParams) {
    msg.author.send(`Format error. Type \`\`${msg.prefix}hat\`\` to see usage options.`)
      .then(resolve)
      .catch(reject);
    return;
  }
  ...
  fs.exists(hatFile, (exists) => {
    ...
    if (!exists) {
      msg.author.send(`**${params[0]}** is not a registered hat.`)
        .then(resolve)
        .catch(reject);
      return;
    }
...
function downloadAvatar(hatData) {
  return new Promise((resolve, reject) => {
    request.get(hatData.avatarUri, { encoding: null })
      .then(response => {
