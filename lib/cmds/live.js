...
exports.run = (client, msg) => new Promise(async (resolve, reject) => {
  ...
  if (msg.channel.type === 'dm') {
    msg.author.send(`**${strings.length}** of Anne's subscribers are currently live.\n` +
      `<http://annemunition.tv/armory>\n\n${strings.join('\n')}`, { split: true }).then(resolve).catch(reject);
  } else {
    msg.channel.send(`**${strings.length}** of Anne's subscribers are currently live.\n` +
      `<http://annemunition.tv/armory>\n(DM me, the bot, for the full list.)`).then(resolve).catch(reject);
  }
});
