...
exports.run = (client, msg, params = []) =>
  new Promise((resolve, reject) => {
    ...
    msg.channel
      .send(`Searching TMDb  for \`\`${params.join(' ')}\`\`...`)
      .then(async (m) => {
        // Create the uri to search the movieDB with the text from discord
        const searchUri = client.utils.buildUri(
        ...
        // Search the movieDB
        const searchResults = client.utils.get(
          ['body'],
          await request.get(searchUri),
        )
        ...
        const movieUri = client.utils.buildUri(
          `https://api.themoviedb.org/3/movie/${searchResults.results[0].id}`,
          {
            api_key: client.config.movieDB.apiKey,
          },
        )
        ...
        if (!movie) {
          m.edit(`Error getting results for: ${params.join(' ')}`)
            .then(resolve)
            .catch(reject)
          return
        }
        ...
        str += '```'
        const embed = new client.Discord.MessageEmbed().setDescription(str)
        ...
        if (movie.imdb_id) {
          embed.setTitle(`http://www.imdb.com/title/${movie.imdb_id}/`)
        }
        // Send the embed and remove the waiting message at the same time
        Promise.all([msg.channel.send({ embed }), m.delete()])
          .then(resolve)
          .catch(reject)
      })
      .catch(reject)
