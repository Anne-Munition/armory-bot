...
exports.run = (client, msg) =>
  new Promise((resolve, reject) => {
    ...
    let status = msg.author.presence.status
      ? client.utils.capitalize(msg.author.presence.status)
      : null
    ...
    str += `Status: ${status}\n`
    str += `Game: ${
      msg.author.presence.game ? msg.author.presence.game.name : null
    }\n`
    ...
    str += `\n--Joined--\n`
    let d = moment
      .duration(moment() - msg.author.createdAt)
      .asMonths()
      .toFixed(0)
    str +=
      `Discord_App: '${moment(msg.author.createdAt).utc().format()}' ` +
      `(${d}+ month${d === 1 ? '' : 's'})\n`
    if (msg.channel.type === 'text') {
      d = moment
        .duration(moment() - msg.member.joinedAt)
        .asMonths()
        .toFixed(0)
      str +=
        `This_Server: '${moment(msg.member.joinedAt).utc().format()}' ` +
        `(${d}+ month${d === 1 ? '' : 's'})\n`
    }
    str = `\`\`\`qml\n${str}\`\`\``
