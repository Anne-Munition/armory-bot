...
exports.run = (client, msg, params = []) => new Promise((resolve, reject) => {
  ...
  msg.channel.sendMessage(`Looking up **${plat}** OW stats for **${name}**. Please wait...`)
    .then(m => {
      ...
      Promise.all(promiseAllUris)
        .then(results => {
          ...
          if (!platformResults) {
            ...
            msg.channel.sendMessage(`No stats found for **${name}**. Capitalization does count.`)
          ...
          if (!highestRegion.level) {
            m.delete().catch(client.logger.error);
            msg.channel.sendMessage(`**${name}** has an Overwatch profile, but has no time played.`)
              .then(resolve).catch(reject);
            return;
          }
          ...
          msg.channel.sendEmbed(new client.Discord.RichEmbed()
            ...
            .setThumbnail(extractedData.avatar)).then(resolve).catch(reject);
        })
        .catch(err => {
          m.delete().catch(client.logger.error);
