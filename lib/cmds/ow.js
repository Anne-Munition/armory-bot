  ...
  Promise.all(statsPromiseArray)
    .then(statsResults => {
      ...
      const scrapePromiseArray = statsResults.map(stats => {
        count++;
        return scrapeData(client, stats, owData[count]);
      });
      Promise.all(scrapePromiseArray)
        .then(scrapeResults => {
          ...
          processResults(client, finalStats, msg).then(resolve).catch(reject);
        })
        .catch(err => {
...
function scrapeData(client, stats, data) {
    ...
    if (stats.quickPlayStats) {
      for (const hero in stats.quickPlayStats.topHeros) {
        if (stats.quickPlayStats.topHeros.hasOwnProperty(hero)) {
          qpHeroes.push({ name: client.utils.capitalize(hero), data: stats.quickPlayStats.topHeros[hero] });
        }
      }
    }
    ...
    if (stats.competitiveStats) {
      for (const hero in stats.competitiveStats.topHeros) {
        if (stats.competitiveStats.topHeros.hasOwnProperty(hero)) {
          rankedHeroes.push({ name: client.utils.capitalize(hero), data: stats.competitiveStats.topHeros[hero] });
        }
      }
    }
...
function processResults(client, finalStats, msg) {
  return new Promise((resolve, reject) => {
    ...
    let code = `\`\`\`qml\n${str}\`\`\``;
    if (client.config.overwatch.hide_battle_tags.indexOf(finalStats.tag) === -1) {
