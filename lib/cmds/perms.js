...
exports.run = (client, msg, params = []) => new Promise(async (resolve, reject) => {
  ...
  if (!client.commands.has(cmd)) {
    msg.reply(`Cannot set or alter permissions for commands that do not exist or are nor loaded. ` +
      `\`\`${msg.prefix}cmds\`\` to see list.`).then(resolve).catch(reject);
    return;
  }
  let result;
  try {
  ...
  if (params.length < 2 || (params[1] && params[1] === 'id')) {
    ...
    msg.channel.send(JSON.stringify(obj, null, 2), { code: 'json' }).then(resolve).catch(reject);
    return;
  }
  ...
  targetToId(client, msg, data, (err, id) => {
    if (err) {
      client.logger.error(err);
      msg.reply('There was an error performing this operation. Please try again.').then(resolve).catch(reject);
    } else {
      data.id = id;
      client.logger.debug('target to id:', data.id);
      alterDbEntry(client, msg, data)
        .then(() => {
          msg.reply('The permissions operation completed successfully.').then(resolve).catch(reject);
        })
        .catch(e => {
...
function targetToId(client, msg, data, callback) {
  ...
  if (data.type === 'member') {
    ...
    if (matchId) {
      ...
      const userMatches = msg.guild.members.filter(m => m.user.username.toLowerCase().includes(data.target));
      const nickMatches = msg.guild.members.filter(m => m.nickname && m.nickname.toLowerCase().includes(data.target));
    ...
    if (matchId) {
      ...
      matches = msg.guild.channels.filter(c => c.name.toLowerCase().includes(data.target));
    ...
    if (matchId) {
      ...
      matches = msg.guild.roles.filter(r => r.name.toLowerCase().includes(data.target));
  ...
  userMatchWait(msg, data, matches)
    .then(id => {
      if (id) {
        ...
        return callback(`No Discord '${data.type}' id match found for **${data.target}**`);
      }
    })
    .catch(err => {
      client.logger.error(err);
