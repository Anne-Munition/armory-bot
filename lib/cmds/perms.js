...
exports.run = (client, msg, params = []) => new Promise(async(resolve, reject) => {
  if (msg.channel.type === 'dm') {
    msg.reply('Unable to check the **perms** from DM channels.');
    return resolve();
  }
  if (params.length === 0) {
  ...
  let result;
  try {
    result = await client.mongo.perms.findOne({ server_id: msg.guild.id, cmd: cmd });
  } catch (err) {
  ...
  logger.debug(cmd, action, type, target);
  // Resolve the TARGET to a Discord ID
  const data = { cmd, action, type, target };
  try {
    data.id = await targetToId(msg, data);
  } catch (err) {
    if (err.discordReply) {
  ...
  logger.debug('target to id:', data.id);
  try {
    await alterDbEntry(client, msg, data);
  } catch (err) {
    if (err) {
      logger.error(err);
      msg.channel.sendMessage('There was an error saving the permission to the database. Please try again.');
      return reject(err);
    }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    if (data.type === 'member') {
      ...
      if (matchId) {
        ...
        result = msg.guild.members.get(matchId[1]);
        if (result) {
          return resolve(result.id);
        } else {
          return reject({ discordReply: 'No match found.' });
        }
      } else {
        ...
        if (matches.size === 0) {
          return reject({ discordReply: 'No matches found.' });
        }
        if (matches.size === 1) {
          return resolve(matches.first().id);
        }
      }
      ...
      if (matchId) {
        ...
        matches = msg.guild.channels.filter(c => c.name.toLowerCase() === data.target);
      ...
      if (matchId) {
        ...
        result = msg.guild.roles.get(matchId[1]);
        if (result) {
          return resolve(result.id);
        } else {
          return reject({ discordReply: 'No match found.' });
        ...
        return resolve(msg.guild.id);
      } else {
        // Target was not a mention, search role names
        matches = msg.guild.roles.filter(r => r.name.toLowerCase() === data.target);
    ...
    test(msg, data, matches)
      .then(resolve)
      .catch(err => {
        err.discordReply = `No Discord '${data.type}' id match found for **${data.target}**`;
        return reject(err);
      });
    return null;
  });
...
function test(msg, data, matches) {
  return new Promise((resolve, reject) => {
    ...
    msg.channel.sendMessage(str)
      .then(matchMessage => {
        ...
        collector.on('end', (collected, reason) => {
          if (reason === 'time') {
            matchMessage.delete();
            return reject({ discordReply: 'You took to long to reply.' });
          }
          if (reason === 'matchesLimit') {
            const num = parseInt(collected.first().content);
            if (num < 1 || num > matches.size) {
              return reject({ discordReply: 'The number you entered is out of range.' });
            }
            matchMessage.delete();
            collected.first().delete();
            return resolve(matches.map(m => m.user.id)[num - 1]);
          }
          return null;
        });
      });
