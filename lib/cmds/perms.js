...
exports.run = (client, msg, params = []) => new Promise(async(resolve, reject) => {
  ...
  try {
    ...
    msg.reply(`Error getting permissions for **${cmd}**.`).then(resolve).catch(reject);
    return null;
  }
  ...
  if (params.length < 2 || (params[1] && params[1] === 'id')) {
    ...
    msg.channel.sendCode('json', JSON.stringify(obj, null, 2)).then(resolve).catch(reject);
    return null;
  }
...
function alterDbEntry(client, msg, data) {
  return new Promise(async(resolve, reject) => {
    ...
    result.save().then(resolve).catch(reject);
    return null;
  });
}
