...
exports.run = (client, msg, params = []) => new Promise((resolve, reject) => {
  ...
  const players = msg.guild.members
    .filter(m => m.user.presence.activity && m.user.presence.activity.type === 'PLAYING' &&
      m.user.presence.activity.name.toLowerCase().includes(game.toLowerCase()))
    .map(m => {
      return { name: m.user.username, modified: m.user.username.toLowerCase(), game: m.user.presence.activity.name };
    })
    .sort((a, b) => {
