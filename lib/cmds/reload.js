...
exports.run = (client, msg, params = []) =>
  new Promise((resolve, reject) => {
    ...
    if (msg.author.id !== client.config.owner_id) {
      msg
        .reply('Only the bot owner has permissions to use ``reload``.')
        .then(resolve)
        .catch(reject)
      return
    }
    ...
    if (params[0] === 'twitterclient') {
      ...
      client.logger.info('Manually resetting twitter client')
    ...
    client.utils
      .loadOneCommand(client, params[0])
      .then(() => {
        msg
          .reply(`Command **${params[0]}** loaded successfully`)
          .then(resolve)
          .catch(reject)
      })
      .catch(reject)
