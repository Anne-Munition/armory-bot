...
exports.run = (client, msg, params = []) => new Promise((resolve, reject) => {
  ...
  const embed = new client.Discord.RichEmbed()
    ...
    .setThumbnail('http://1.bp.blogspot.com/-fRzdpILdENU/TXXL0JQtIOI/AAAAAAAAADE/ngpY1z88iZ8/s1600/d20.jpg');
  // Only a number
  const numberOnly = new RegExp('^[1-9]\\d*$');
  if (numberOnly.test(query)) {
    embed.addField('Result:', `**${client.utils.getRandomInt(1, parseInt(query) + 1)}**`);
  ...
  if (dice.length < 2 && dice[0].text.split('d')[0] === 1) {
    ...
    if (discordExploded.join('').length < 500) {
      embed.addField('Dice:', discordExploded.join(''));
    }
    embed.addField('Critical Failures:', critFails, true)
      .addField('Critical Successes:', critSuccess, true)
      .addField('Lowest Roll:', lowest, true)
