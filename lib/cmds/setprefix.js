...
exports.run = (client, msg, params = []) =>
  new Promise((resolve, reject) => {
    ...
    // Exit if not bot owner or build owner
    if (
    ...
    client.mongo.guildConfigs
      .update({ server_id: msg.guild.id }, data, { upsert: true })
      .then(() => {
        client.logger.debug(
          `Saved setPrefix ${conf.prefix} for guild ${msg.guild.name}`,
        )
        msg
          .reply(
            `you have changed the prefix of all commands on this server to: \`\`${conf.prefix}\`\``,
          )
          .then(resolve)
          .catch(reject)
      })
