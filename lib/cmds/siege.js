...
exports.run = (client, msg, params = []) => new Promise((resolve, reject) => {
  ...
  const token = new Buffer.from(`${client.config.ubi.email}:${client.config.ubi.password}`).toString('base64');
  msg.channel.send(`Looking up **${platform}** RB6 Siege stats for **${name}**. Please wait...`)
    .then(async m => {
      try {
        ...
        const operatorsList = (await fetch('https://game-rainbow6.ubi.com/assets/data/operators.bbbf29a090.json')).body;
        if (!operatorsList) {
          msg.channel.send(`Unable to get operator list.`);
          return;
        }
        let operators = [];
        for (let operator in operatorsList) {
          if (operatorsList.hasOwnProperty(operator)) {
            operators.push({ index: operatorsList[operator].index, name: operator})
          }
        }
        const opTimePlayed = [];
        for (const key in data) {
          if (data.hasOwnProperty(key)) {
            if (key.startsWith('operatorpvp_timeplayed')) {
              const opIndex = get(key.match(/operatorpvp_timeplayed:(.:.):infinite/), '[1]', null);
              if (!opIndex) continue;
              const op = operators.find(operator => operator.index === opIndex)
              if (!op) continue;
              opTimePlayed.push({ name: titleCase(op.name), time: data[key] });
            }
          }
        }
        const statistics = {
