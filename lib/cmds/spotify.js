...
exports.run = (client, msg, params = []) => {
  ...
  getID(msg, id, params)
    ...
    .then(track => spotify.addTrack(client, track))
    .then(track => {
      msg.reply(`:notes: '**${track.name}** - ${track.artists[0].name}' has been added to Anne's playlist.`);
    })
    .catch(err => {
      ...
      .then(result => {
        if (result) {
          if (result.count >= config.spotify.max_count) {
            reject({ discordReply: `This track has been submitted over ` +
          ...
          result = client.mongo.spotifyTracks({ track: track.uri, count: 1 });
        }
        result.save()
          .then(() => {
            resolve(track);
