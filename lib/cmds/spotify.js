  ...
  if (params[0].toLowerCase() === 'isauth') {
    spotify.authenticate()
      ...
      .catch(err => {
        ...
        msg.reply(`:thumbsdown: The authentication to spotify has failed. Run` +
          ` \`\`${msg.prefix}spotify auth\`\` to generate a new token.`);
      });
    return;
  }
  ...
  if (params[0].toLowerCase() === 'who') {
    ...
    client.mongo.spotifyTracks.findOne({ uri: `spotify:track:${id}` })
      .then(result => {
        ...
        let str = `The track '**${result.track.replace(' - ', '** - ')}' was submitted by:\n`;
        let count = 0;
        result.submitters
