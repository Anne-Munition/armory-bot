'use strict';
exports.info = {
  desc: 'Add a track to a Spotify playlist to share with AnneMunition.',
  usage: '',
  aliases: [],
};
...
exports.run = (client, msg, params = []) => {
  ...
  if (params.length === 0) {
    msg.reply('Please enter a Spotify URL, Spotify ID, or track name.\n' +
      `\`\`${msg.prefix}spotify <url | id | name>\`\``);
    return;
  }

  const owner = (msg.channel.type === 'dm') ||
    msg.author.id === msg.guild.owner.id ||
    msg.author.id === config.owner_id;

  if (owner && params[0].toLowerCase() === 'enable') {
    const details = spotify.getDetails();
    details.enabled = true;
    spotify.setDetails(details);
    msg.reply('The Spotify service has been enabled.');
    return;
  }

  if (owner && params[0].toLowerCase() === 'disable') {
    const details = spotify.getDetails();
    details.enabled = false;
    spotify.setDetails(details);
    msg.reply('The Spotify service has been disabled.');
    return;
  }

  if (!spotify.getDetails().enabled) {
    msg.reply('The Spotify service is currently disabled.');
    return;
  }

  // Method for user to check if the app is currently authorized
  if (owner && params[0].toLowerCase() === 'isauth') {
    spotify.authenticate()
      .then(() => {
        msg.reply(':thumbsup: Successful authentication to the Spotify API.');
      })
      .catch(err => {
        if (err && err.message === 'invalid_request') {
          logger.debug('invalid_request');
        } else if (err) {
          logger.error(err);
        }
  ...
  if (owner && params[0].toLowerCase() === 'auth') {
    // Get auth uri
    setToken(msg);
    return;
  }

  if (owner && params[0].toLowerCase() === 'id') {
    // Save playlist ID
    if (!params[1]) {
      msg.reply(`\`\`${msg.prefix}spotify id <playlistid>\`\``);
      return;
    }
    const id = matchID(params[1]);
    if (!id) {
      msg.reply('That is not a known Spotify ID.');
      return;
    }
    spotify.resolvePlaylist(id)
      .then(() => {
        msg.reply('Playlist added, ready to add tracks.');
      })
      .catch(err => {
        if (err && err.discordReply) {
          msg.reply(`${err.discordReply} \`\`${msg.prefix}spotify auth\`\``);
        } else if (err) {
          logger.error(err);
        }
      });
  ...
  let str = 'Spotify has not yet finished being setup to run on this server.';
  if (!spotify.getDetails().refreshToken) {
    if (owner) {
      str += '\nSending you a DM to generate a token.';
      setToken(msg);
    }
    msg.reply(str);
    return;
  ...
  if (!spotify.getDetails().playlistId) {
    if (owner) {
      str += `\nMissing playlist ID. Please run \`\`${msg.prefix}spotify id <playlistid>\`\` to save one.`;
    }
    msg.reply(str);
    return;
  }
      ...
      .then(result => {
        if (result) {
          if (result.count >= config.spotify.max_count) {
            reject({
              discordReply: `This track has been submitted ` +
...
function setToken(msg) {
  ...
  const str = `Please follow the link below and copy/paste the displayed code into this chat.\n` +
    `We will wait for 2 minutes for the code to be entered.\n\n<${uri}>\n\n` +
  ...
  msg.author.sendMessage(str)
    .then(m => {
      ...
      collector.on('end', (collected, reason) => {
        if (reason === 'time') {
          msg.author.sendMessage('The 2 minute timer has expired.');
          return;
        }
        if (reason === 'matchesLimit') {
          ...
          spotify.setAuthCode(code)
            .then(() => {
              ...
              let s = ':thumbsup: Thanks! That worked.\n';
              if (spotify.getDetails().playlistId) {
                s += 'Members should now be able to add tracks to your playlist.';
              } else {
                s += 'We still need to enter a Spotify playlist id to save the tracks to.' +
                  `\nRun \`\`${msg.prefix}spotify id <playlistid>\`\``;
              }
              msg.author.sendMessage(s);
            })
            .catch(err => {
...
function matchID(str) {
  ...
  const match = str.match(spotifyId);
  if (match && match[1]) {
    return match[1];
  } else {
    return null;
