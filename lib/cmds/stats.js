...
exports.run = (client, msg) => new Promise((resolve, reject) => {
  ...
  pusage.stat(process.pid, (err, stat) => {
    ...
    if (err) {
      ...
      str += '\n--System Stats--\n';
      str += `OS: '${utils.capitalize(os.type().replace('_NT', ''))} - ${os.release()}`;
      if (os.platform() === 'linux') {
        str += ` - ${utils.capitalize(thisOS)}`;
      }
      str += '\'\n';
      ...
      //str += '\n--Twitter Stats--\n';
      //str += `Users: ${client.twitter.ids.length}\n`;
      //str += `Channels: ${client.twitter.channels}\n`;
      // TODO: display these stats once available
      //str += '\n--Twitch Stats--\n';
      //str += `Users: ${client.twitch.users}\n`;
      //str += `Channels: ${client.twitch.channels}\n`;
      ...
      msg.channel.sendCode('qml', str).then(resolve).catch(reject);
    }
  });
});
