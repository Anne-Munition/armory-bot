  ...
  if (result) {
    ...
    if (chan.length === 0) {
      ...
      result.save({ upsert: true })
        .then(() => {
          msg.channel.sendMessage(`This channel will now receive posts when **${result.display_name}** ` +
            `goes live on Twitch.\nThis will **not** trigger until the channel next goes live.`);
        })
        .catch(err => {
          logger.error(err);
    ...
    getTwitchUser(params[1], msg)
      .then(data => {
        if (data) {
          ...
          entry.save()
            .then(() => {
              msg.channel.sendMessage(`This channel will now receive posts when **${data.display_name}** ` +
                `goes live on Twitch.\nThis will trigger in a few minutes if the channel is currently live.`);
            })
            .catch(logger.error);
        }
      })
...
function getListHeader(name) {
  return `Posting when **${name}** goes live on Twitch to:\n`;
}
