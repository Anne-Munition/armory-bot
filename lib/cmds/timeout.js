...
exports.run = (client, msg, params = []) => new Promise(async (resolve, reject) => {
  ...
  if (msg.channel.id !== '707788445774512128') return;
  ...
  if (!target) {
    msg.channel.send('Nobody with that ID was found. Are you sure it is correct?').then(resolve).catch(reject);
    return;
  }
  ...
  if (isNaN(minutes)) {
    ...
    msg.channel.send('The duration must be a whole number.').then(resolve).catch(reject);
    return;
  }
  ...
  target.addRole('706906565784895509').then(() => {
    ...
    msg.channel.send(`**${targetString}** has been timed out for **${minutes}** minute(s) by ${msg.author.username}`);
