    ...
    if (result) {
      ...
      if (chan.length === 0) {
        ...
        result.save({ upsert: true })
          .then(() => {
            let n = '';
            if (client.twitch.waiting.indexOf(result.display_name.toLowerCase()) !== -1) n = getWarningMessage();
      ...
      client.twitch.getUserData(query)
        .then(user => {
          if (!user) {
            msg.channel.send(`**${query}** is not a known Twitch channel.`)
              .then(resolve).catch(reject);
          } else {
            const data = {
