    ...
    if (params[1] === 'all' && msg.author.id === client.config.owner_id) {
      ...
      results.forEach(result => {
        const channels = result.channels.map(c => client.channels.get(c.channel_id))
          .filter(x => x)
          .map(x => `${client.utils.removeFormatting(x.guild.name)} - **#${client.utils.removeFormatting(x.name)}**`);
        if (channels.length > 0) {
          str += `**${client.utils.removeFormatting(client.utils.makePossessive(result.screen_name))}** ` +
      ...
      results.filter(n => {
        ...
        .forEach(n => {
          ...
          if (channels.length > 0) {
            str += `**${client.utils.removeFormatting(client.utils.makePossessive(n.screen_name))}** ` +
              `tweets are posted to:\n`;
            str += channels.join('\n');
            str += '\n\n';
          }
  ...
  return new Promise(async(resolve, reject) => {
    if (result) {
      ...
      msg.channel.sendMessage(`This channel will now receive tweets from ` +
        ...
        .then(() => {
          const collector = new client.Discord.MessageCollector(msg.channel,
