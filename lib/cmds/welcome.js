...
exports.run = (client, msg, params = []) => new Promise(async(resolve, reject) => {
  ...
  if (params[0] === 'list') {
    ...
    if (owner && params[1] === 'all') {
      ...
      if (names.length === 0) {
        msg.channel.send('No channels currently post a welcome message.')
    ...
    if (names.length === 0) {
      msg.channel.send('No channels on this server currently post a welcome message.')
        .then(resolve).catch(reject);
      return;
    }
    str += names.join('\n');
...
function addChannel(client, msg, channels) {
  return new Promise((resolve, reject) => {
    if (channels.length === 0) {
      ...
      entry.save()
        .then(() => {
          msg.channel.send('This channel will now post welcome messages.')
            .then(resolve).catch(reject);
        })
        .catch(err => {
      ...
      msg.channel.send('This channel already posts welcome messages.')
        .then(resolve).catch(reject);
    }
  });
}
...
function removeChannel(client, msg, channels) {
  return new Promise(async(resolve, reject) => {
    if (channels.length === 0) {
      msg.channel.send('This channel doesn\'t currently post welcome messages.')
        .then(resolve).catch(reject);
      return;
    }
    try {
    ...
    msg.channel.send('This channel will no longer post welcome messages.')
      .then(resolve).catch(reject);
  });
}
