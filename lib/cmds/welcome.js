  ...
  new Promise(async (resolve, reject) => {
    ...
    if (params[0] === 'list') {
      ...
      if (owner && params[1] === 'all') {
        ...
        if (names.length === 0) {
          msg.channel
            .send('No channels currently post a welcome message.')
            .then(resolve)
            .catch(reject)
          return
        }
        ...
        names = channels
          .map((c) => msg.guild.channels.cache.get(c.channel_id))
          .filter((x) => x)
          .sort((a, b) => a.position - b.position)
          .map((x) => x.toString())
      }
      if (names.length === 0) {
        msg.channel
          .send('No channels on this server currently post a welcome message.')
          .then(resolve)
          .catch(reject)
        return
      }
      ...
      let channels
      try {
        channels = await client.mongo.welcomeChannels.find({
  ...
  return new Promise((resolve, reject) => {
    if (channels.length === 0) {
      ...
      entry
        ...
        .then(() => {
          msg.channel
            .send('This channel will now post welcome messages.')
            .then(resolve)
            .catch(reject)
        })
        .catch((err) => {
          client.logger.error(
            'Error Saving welcome channel to the mongoDB',
            err,
          )
          msg.channel.send('There was a database error. Please try again.')
          reject(err)
      ...
      msg.channel
        .send('This channel already posts welcome messages.')
        .then(resolve)
        .catch(reject)
    }
  })
...
function removeChannel(client, msg, channels) {
  return new Promise(async (resolve, reject) => {
    if (channels.length === 0) {
      msg.channel
        .send("This channel doesn't currently post welcome messages.")
        .then(resolve)
        .catch(reject)
      return
    }
    try {
      ...
      client.logger.error(
        'Error removing welcome channel from the mongoDB',
        err,
      )
      msg.channel.send('There was a database error. Please try again.')
      reject(err)
    ...
    msg.channel
      .send('This channel will no longer post welcome messages.')
      .then(resolve)
      .catch(reject)
  })
}
