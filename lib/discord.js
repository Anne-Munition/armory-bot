...
module.exports = function DiscordClient() {
  ...
  fs.readdir(cmdsPath, (err, files) => {
    if (err) {
      ...
      logger.info(`${commands.size} command(s) loaded.`);
    }
  });
  ...
  discord.on('ready', () => {
    ...
    // Send readied discord client to chess; starts chess games in channels
    myEvents.emit('discord_ready', discord);
  });
  ...
  // Chess games are loaded, get discord object to start any bot turns
  // This should only happen if it takes a really long time to load all the chess games
  // As normally this is ready long before Discord is ready
  myEvents.on('chess_ready', () => {
    if (discord.user) {
      myEvents.emit('discord_ready', discord);
    }
  });
