...
discord.client.on('guildMemberAdd', member => {
  ...
  mongo.welcomeChannels.find({ server_id: member.guild.id }, (err, channels) => {
    if (err) {
      ...
      channels.forEach(c => {
        // Resolve the channel
        const channel = member.guild.channels.find('id', c.channel_id);
        if (channel) {
...
discord.client.on('guildMemberRemove', member => {
  // Get list of welcome channels with same server_id
...
function prefixCommands(msg) {
  ...
  const cmd = params.shift().slice(config.commands.prefix.length).toLowerCase();
    ...
    perms.check(mongo, msg, cmd)
      .then(allowed => {
        if (allowed || cmd === 'perms') {
          ...
          discord.client.count.commands++;
          command.run(discord, msg, params, mongo, allowed);
        } else {
...
myEvents.on('tweet', data => {
  ...
  function postToChannel(c) {
    ...
    if (data.media.length > 0) {
      const m = data.media.shift();
      const loc = m.video && !m.video.includes('http') ? m.video : m.image;
      channel.sendFile(loc, path.parse(loc).base, data.str)
        .then(headerMsg => {
          remainingMedia(headerMsg);
        })
        .catch(() => {
          next();
        });
  ...
  function remainingMedia(msg) {
    msgIds.push({ channel: msg.channel.id, message: msg.id });
    data.media.forEach(media => {
      const m = media.video && !media.video.includes('http') ? media.video : media.image;
      msg.channel.sendFile(m, path.parse(m).base)
        .then(mediaMsg => {
