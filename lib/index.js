...
const spoilerTags = require('./spoilerTags')
require('./memoryUsage')
...
// circular Discord object for MessageEmbed
...
client.once('ready', () => {
  ...
  // restart any timeout timers
  // pass the client so it may send with only an user id to work with
  timeouts.init(client).catch()
})
...
client.on('message', (msg) => {
  // Do stuff with messages from a bot

  // AnneMunition Schedule Auto Pinning
  schedulePinning(msg)

  // Don't listen to bots after this point
  ...
  spoilerTags(msg).catch()

  // Do other stuff with messages not from a bot below...
})
...
async function schedulePinning(msg) {
  ...
  if (msg.channel.id === '362349719663542272') {
    ...
    if (contentHasScheduleTerms(msg)) {
      ...
      // const newestMessages = await msg.channel.messages.fetch({ after: msg.id });
...
function prefixedCommands(msg) {
  ...
  if (msg.channel.type === 'text') {
    ...
    if (!msg.content.startsWith(conf.prefix)) {
      ...
      const anyConfig = client.guildConfigs.find((x) =>
        msg.content.startsWith(x.prefix),
      )
      if (!anyConfig) return
  ...
  if (!command || !command.run || typeof command.run !== 'function') {
    logger.debug(
      `The cmd '${cmd}' does not exist, is not loaded, or is missing the 'run' function`,
    )
    return
  }
  ...
  if (
    ...
    logger.error(
      'We do not have the required permissions in this channel to execute the command to completion.',
    )
    return
  }
  ...
  if (allowed) {
    ...
    command
      .run(client, msg, params)
      .then(() => {
        // If the command completes OK run cleanup tasks
        logger.info(
          `${cmd} finished in: ${(now() - msg.startTime).toFixed(3)}ms`,
        )
      })
      .catch((err) => {
