...
async function prefixedCommands(msg) {
  ...
  // Store command name to message object
  msg.cmd = cmd;
  ...
  logger.debug('Command Needed Permissions', command.info.permissions);
  // Ensure the bot client has every permissions required to run this command on Discord's end
  if (!command.info.permissions.every(perm => msg.channel.permissionsFor(client.user).has(perm))) {
  ...
  if (allowed) {
    ...
    command.run(client, msg, params)
      .then(() => {
        // If the command completes OK run cleanup tasks such at time to completion
        client.utils.finish(msg, cmd);
      })
      .catch(err => {
