...
async function prefixedCommands(msg) {
  ...
  // Save the prefix used on this message in the message object to build guild specific help commands
  ...
  if (msg.channel.type === 'text' && !msg.channel.permissionsFor(client.user).has('SEND_MESSAGES')) {
    logger.error('We do not have SEND_MESSAGES permissions in this channel. Exiting...');
    return;
  }
