...
module.exports = function (utils, options, discord, request, mongo, twitter, fs, path, moment) {
    ...
    function ow(message, query) {
        ...
        request.get({url: base_url}, (err, res, body) => {
            ...
            let data = [];
            let level = $('.player-level').text();
            let game_hours = $($($($('div[data-group-id=stats]').children()[6]).find('tbody').children()[5]).children()[1]).text();
            let heroes = $('.progress-category')[0];
            for (let i = 0; i < 5; i++) {
                let hero = $(heroes).children()[i];
                let name = $(hero).find('.title').text();
                let time = $(hero).find('.description').text();
