...
module.exports = function (utils, options, discord, request, mongo, twitter, fs, path, moment) {
    ...
    function shame(message) {
        discord.quickFile(message, "./assets/shame.gif", "shame.gif");
    }
    ...
    function restart(message) {
        if (message.author.id == "84770528526602240" || message.author.id == "84761911010267136") {
            process.exit(2);
        }
    }
    ...
    function showTweets(message, query) {
        if (!options.twitter.enabled) return;
        if (query[0] == 'list') {
            ...
            mongo.twitterChannelsModel.find({channel_id: message.channel.id}, (err, channels) => { //Search database for existing entry
                ...
                switch (query[0]) {
                    ...
                    case 'remove':
                        ...
                        break;
                    default:
                        discord.typeMessage(message, "``" + options.command_prefix + "showtweets < list | add | remove >``");
                        break;
                }
            });
        }
    ...
    function hat(message, query) {
        ...
        if (!message.channel.isPrivate) {
            discord.typeReply(message, "Sending PM...");
            sendHatFormatMessage();
        }

        function sendHatFormatMessage() {
            discord.typeMessage(message.author, "``" + options.command_prefix + "hat <name> <Xoffset> <Yoffset> <scale> <rotation>``\n" +
        ...
        let hatFile = path.join(process.cwd(), "assets/hats/" + query[0] + ".png");

        fs.exists(hatFile, (exists) => {
            if (!exists) {
                discord.typeMessage(message, "'" + query[0] + "' is not a registered hat.");
            } else {
                let hatData = {
                    hatFile: hatFile,
                    x: x,
        ...
        function saveModifiedHat(hatData) {
            return new Promise((resolve, reject)=> {
                let tempHatPath = path.join(process.cwd(), "temp/hat_" + message.author.id + "_" + query[0] + ".png");
