...
module.exports = function (options, discord, mongo, twitter) {
    ...
    var nodecraftAPI = nodecraft(options.nodecraft.username, options.nodecraft.api_key);
    ...
    function ow(message, query) {
        ...
        function scrapeData(html, url, region) {
            ...
            let prestige_border = $('.player-level').attr('style').toString();
            let border_img = prestige_border.match(/\d\w.*\.png/)[0];
            let prestige_rank = $('.player-rank').attr('style').toString();
            let rank_img = prestige_rank.match(/\d\w.*\.png/)[0];
            if (border_img == "0x0250000000000926_Border.png") {
                if (rank_img == "0x0250000000000926_Rank.png") {
                    level += 100;
                } else if (rank_img == "0x0250000000000923_Rank.png") {
                    level += 200;
                } else if (rank_img == "0x0250000000000934_Rank.png") {
                    level += 300;
                } else if (rank_img == "0x0250000000000935_Rank.png") {
                    level += 400;
                } else if (rank_img == "0x025000000000092F_Rank.png") {
                    level += 500;
                }
            } else if (border_img == "0x0250000000000956_Border.png") {
                level += 600
            }
    ...
    function minecraft(message) {
        nodecraftAPI.services.stats(options.nodecraft.server_id, (err, results) => {
            if (!err) {
                ...
                discord.client.sendMessage(message, response);
            } else {
                discord.client.sendMessage(message, "There was an error getting data for the subscriber server.")
            }
        });
    }
};
