...
module.exports = function (utils, options, discord, request, mongo, twitter, fs, path, moment, jsonfile, queueList) {
    ...
    function ow(message, query) {
        ...
        if (tag.indexOf("#") == -1) return discord.client.sendMessage(message, "Invalid format for lookup. ``" + options.commands.prefix + "ow name#id``");
        let us_url = encodeURI("https://playoverwatch.com/en-us/career/pc/us/" + tag + "/").replace("#", "-");
        let eu_url = encodeURI("https://playoverwatch.com/en-us/career/pc/eu/" + tag + "/").replace("#", "-");
        let name = tag.split("#")[0];

        getStatsPage(us_url)
            .then((body) => {
                scrapeStats(body, us_url);
            }, () => {
                getStatsPage(eu_url)
                    .then((body) => {
                        scrapeStats(body, eu_url);
                    }, () => {
                        return discord.typeReply(message, "Error getting stats for ``" + name + "``");
                    });
            })
            .catch((err) => {
                console.log(err);
        ...
        function getStatsPage(url) {
            return new Promise((resolve, reject) => {
                request.get(url, (err, res, body) => {
                    if (err || res.statusCode != 200) reject();
                    resolve(body);
                });
            })
        }
        
        function scrapeStats(html, url) {
            ...
            if (game_hours == "") return discord.typeMessage(message, "``" + name + "`` has an Overwatch profile, but has no time played.");
            ...
            let str = "```" + name + (name.endsWith("s") ? "'" : "'s") + " Overwatch level: " + level + ". Time Played: " + game_hours + ". Top heroes: ```";
