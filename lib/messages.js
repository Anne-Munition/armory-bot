...
module.exports = function (utils, options, discord, request, mongo, twitter, fs, path, moment, jsonfile, queueList) {
    ...
    function ow(message, query) {
        let player = "";
        let id = "";
        if (query.length >= 2) { //<name> #<id>
            player = query[0];
            id = query[1].replace("#", "");
        } else if (query.length == 1) { //<name>#<id> (no space)
            if (query[0].indexOf("#") == -1) {
                discord.typeReply(message, "Format not acceptable for stats lookup. ``name#id``");
                return;
            }
            player = query[0].split("#")[0];
            id = query[0].split("#")[1];
        } else { //Showing stats for Anne if no other player is supplied
            player = "AnneMunition";
            id = "1821";
        }
        let tag = player + "-" + id;
        let base_url = encodeURI("https://playoverwatch.com/en-us/career/pc/us/" + tag + "/");

        request.get({url: base_url}, (err, res, body) => {
            if (err || res.statusCode != 200) {
                return discord.typeReply(message, "Error getting stats for ``" + player + "``");
            }
            ...
            let str = "```" + player + (player.endsWith("s") ? "'" : "'s") + " Overwatch level: " + level + ". Time Played: " + game_hours + ". Top heroes: ```";
            if (player != "AnneMunition") str += "<" + base_url + ">\n"; //Don't show link with Anne's ID
    ...
    function queue(message, query) {
        ...
        if (query.length == 0 || query[0].indexOf("#") == -1) return discord.client.reply(message, "Please provide your battle.net tag. ``" + options.commands.prefix + "queue name#id``");
    ...
    function bump(message, query) {
        ...
        let pos = 0;
        if (query.length != 0) {
            let n = getPosition(queueList, query[0].substring(2, query[0].length - 1));
            if (n != -1) pos = n;
        }
        ...
        saveQueue((err) => {
            if (err) {
                ...
                discord.client.sendMessage(message, message.channel.server.members.get("id", m[0].id) + ", You were removed from the queue.");
            }
        });
    }
    ...
    function next(message, query) {
        ...
        let str = "```Up next to play Overwatch is:```";
