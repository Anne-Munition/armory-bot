...
module.exports = function (options, discord, mongo, twitter) {
    ...
    function owBump(message, query) {
        if (message.channel.id != "188197329356980224") return;
        if (utils.findInArray(message.channel.server.rolesOfUser(message.author), "id", "188173775995535360") == -1 && utils.findInArray(message.channel.server.rolesOfUser(message.author), "id", "84778943529365504") == -1 && message.author.id != "84761911010267136") return;
        mongo.gameQueues.findOne({game: "overwatch"}, (err, result) => {
            if (!err) {
                if (!result) {
                    result = {
                ...
                mongo.gameQueues.update({game: "overwatch"}, {members: result.members}, {upsert: true}, (err) => {
                    if (!err) {
                        let member = message.channel.server.members.get("id", bumped[0].member_id);
                        let str = (message.channel.server.detailsOf(member).nick || member.username) + ", ***" + bumped[0].tag + "*** - was removed from the queue.\n\n";
    ...
    function owNext(message, query) {
        ...
        mongo.gameQueues.findOne({game: "overwatch"}, (err, result) => {
            if (!err) {
                ...
                if (result.members.length == 0) return discord.client.sendMessage(message, "The queue is empty.");
                let count;
                if (query.length != 0 && (utils.findInArray(message.channel.server.rolesOfUser(message.author), "id", "188173775995535360") != -1 || utils.findInArray(message.channel.server.rolesOfUser(message.author), "id", "84778943529365504") != -1) || message.author.id == options.owner_id) {
                    if (query[0] == "*") {
    ...
    function owFlush(message) {
        if (utils.findInArray(message.channel.server.rolesOfUser(message.author), "id", "84778943529365504") == -1 && message.author.id != "84761911010267136" && message.author.id != options.owner_id) return;
    ...
    function getNextList(result, message, count, mention) {
        ...
        for (let i = 0; i < count; i++) {
            str += "``" + (i + 1) + ".`` ";
            let member = message.channel.server.members.get("id", result.members[i].member_id);
            if (i <= 1 && mention) {
                str += member;
            } else {
