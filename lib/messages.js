...
module.exports = function (options, discord, mongo, twitter) {
    ...
    let mod_chat = new RegExp("mod\s?chat", "gmi");
    discord.client.on('message', (message) => { //A message was sent in a Discord channel
        ...
        if (mod_chat.test(message.content)) {
            discord.client.sendMessage(message, "There is no mod chat.");
        }
    });
    ...
    function ow(message, query) {
        ...
        function next(data, overwatch_url, region, platform) {
            ...
            count++;
            if (count >= max) {
                if (Object.keys(results).length === 0 && results.constructor === Object) return discord.typeReply(message, "No stats found for ``" + name + "``");
                let highest_level = 0;
                let highest_name = "";
