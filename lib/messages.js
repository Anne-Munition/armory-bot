...
module.exports = function (utils, options, discord, request, mongo, twitter, fs, path, moment) {
    ...
    discord.client.on('message', (message) => { //A message was sent in a Discord channel
        ...
        if (message.content.startsWith(options.commands.prefix)) handleCommands(message);
    });
    ...
    function ids(message) {
        discord.typeMessage(message,
            ...
            "``Bot User ID: " + discord.client.user.id + " - " + discord.client.user.username + "``\n" +
    ...
    function avatar(message) {
        ...
        request.get({url: encodeURI(url), encoding: null}, (err, res, buffer) => {
            if (!err && res.statusCode == 200) {
                discord.client.sendMessage(message, "``" + message.author.name + "``");
                discord.client.sendFile(message, buffer, message.author.id + ".jpg");
            } else {
    ...
    function hat(message, query) {
        ...
        function sendHatFormatMessage() {
            discord.client.sendMessage(message.author, "``" + options.commands.prefix + "hat <name> <Xoffset> <Yoffset> <scale> <rotation>``\n" +
    ...
    function ow(message, query) {
        ...
        if (query.length >= 2) {
            ...
            if (query[0].indexOf("#") == -1) {
                discord.typeMessage(message, "Format not acceptable for stats lookup.");
                return;
            }
            player = query[0].split("#")[0];
            id = query[0].split("#")[1];
        } else {
            player = "AnneMunition";
            id = "1821";
        ...
        let tag = player + "-" + id;
        let base_url = encodeURI("https://masteroverwatch.com/profile/pc/us/" + tag);
        request.get({url: encodeURI(base_url + "/heroes")},
            (err, res, body) => {
                if (err || res.statusCode != 200) {
                    if (err) {
                    ...
                    discord.typeMessage(message, "Unable to find data for ``" + player + "``");
                    return;
                }
                let $ = cheerio.load(body);
                let hero_list = $('.heroes-list');
                let data = [];

                for (let i = 0; i < 3; i++) {
                    let hero = $(hero_list).children()[i];
                    let rank = $(hero).find('.hero-rank').find('strong').text();
                    let name = $(hero).find('.heroes-icon').find('strong span').text();
                    let time = $(hero).find('.heroes-stats-time').text();
                    data.push({rank: rank, name: name, time: time});
                }
                let str = "```Top 3 played heroes and ranks for " + player + ":```";
                if (player.toLowerCase() != "annemunition") str += "<" + base_url + ">";
                if (hero_list.children().length == 0) {
                    str += "\nNo stats found. Must have visited the url via browser at least once to generate a profile.";
                    discord.typeMessage(message, str);
                    return;
                }
                for (let i = 0; i < data.length; i++) {
                    str += "\n``" + (i + 1) + ".``**" + data[i].name + "** - #" + data[i].rank;
                    str += " - " + data[i].time;
                }
                discord.typeMessage(message, str);
            });
