...
module.exports = function Mongo(client) {
  ...
  mongoose.connection.on('connected', () => {
    ...
    guildConfigs.find({})
      .then(guilds => {
        logger.debug(`Got (${guilds.length}) guild configs on mongo connected`);
    ...
    perms.find({})
      .then(perms => {
        logger.debug(`Got (${perms.length}) command perms on mongo connected`);
        perms.forEach(perm => {
  ...
  const spotifyTracks = mongoose.model('spotify_tracks', {
    uri: String,
    count: Number,
    track: String,
  }, 'spotify_tracks');
