...
module.exports = function Mongo(client) {
  ...
  mongoose.connection.on('connected', () => {
    ...
    chessGames.find({})
      .then(games => {
        logger.debug(`Got (${games.length}) chess games`);
        games.forEach(game => {
          game.data.messages = [];
          client.chessGames.set(game.channel_id, game.data);
        });
      })
      .catch(err => {
        logger.error('Error getting chess games from mongoDB', err);
      });
  });
  ...
  const chessGames = mongoose.model('chess_games', {
    channel_id: String,
    data: {
      game: String,
      w: String,
