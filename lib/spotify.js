...
module.exports = function Spotify() {
  ...
  function removeTrack(client, track) {
    return new Promise((resolve, reject) => {
      ...
      // Search for match in database, case insensitive
      const r = new RegExp(track, 'i');
      client.mongo.spotifyTracks.findOne({ track: { $regex: r } })
        .then(result => {
          if (result) {
            // Authenticate to spotify if we have a match
            authenticate()
              .then(body => {
                ...
                request.delete({
                  ...
                  if (err || res.statusCode !== 200 || !results) {
                    reject(err);
                  } else {
                    resolve();
                  }
    ...
    req.on('success', data => {
      ...
      if (track !== nowPlaying) {
        ...
        removeTrack(client, nowPlaying)
          .then(() => {
            logger.debug('Successful removal of a spotify track from the playlist', track);
          })
          .catch(err => {
            logger.debug('Error removing a spotify track from the playlist', track, err);
          });
        // Store latest track
        nowPlaying = track;
      }
