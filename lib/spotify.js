...
module.exports = function Spotify() {
  function start() {
    setInterval(checkRecentPlaying, 1000 * 10);
    checkRecentPlaying();
  }
  ...
  function removeTrack(track) {
    ...
    search(track)
      .then(rTracks => {
        ...
        const rTrack = rTracks[0].uri;
        logger.debug('Removing track from playlist:', rTrack);
        tokenProvider.getToken((err, accessToken) => {
          if (err) {
            logger.debug('Spotify access_token NOT ok');
          } else {
            ...
            spotifyApi.getMe()
              .then(data => {
                logger.debug('Spotify got me ok');
                // Token is valid, add track
                spotifyApi.removeTracksFromPlaylist(data.body.id, config.spotify.playlistID, { tracks: [rTrack] })
                  .then(() => {
                    logger.debug('Removed track from spotify playlist');
                  })
                  .catch(e => {
  ...
  function checkRecentPlaying() {
    const request = lastfm.request('user.getRecentTracks', { user: 'DBKynd', limit: 1 });
    request.on('error', logger.error);
    request.on('success', data => {
      ...
      logger.debug('Track are the same');
    });
  }
