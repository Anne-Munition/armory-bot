...
const { timeouts: TimeoutsModel } = require('./mongo');
...
async function init(_client) {
  ...
  const entries = await TimeoutsModel.find();
...
async function removeTimeout(id) {
  ...
  const entry = await TimeoutsModel.findOne({ discordId: id });
  ...
  client.channels.get('252275732628242432').sendMessage(`The timeout has ended for **${targetString}**`).catch();
}
