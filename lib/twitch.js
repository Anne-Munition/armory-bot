...
module.exports = function TwitchLive(client) {
  ...
  async function getUserLogo(id) {
    const uri = `https://api.twitch.tv/kraken/users/${id}?api_version=5&client_id=${config.twitch.client_id}`;
    logger.debug(uri);
    const response = await requestJSON(uri);
    logger.debug(response);
    return response.logo || 'https://static-cdn.jtvnw.net/jtv_user_pictures/xarth/404_user_70x70.png';
  }
