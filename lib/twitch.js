          ...
          .then(promiseResults => {
            ...
            if (lastState) {
              ...
              merged.forEach(stream => {
                ...
                const pos = client.twitch.ignore.indexOf(stream.channel.name.toLowerCase());
                if (pos !== -1) {
                ...
                if (inLastState) {
                  ...
                  if (lastGame !== stream.game) {
                    // TODO: Messages on game changes
                    postChange(client, mongoData, stream);
                    return;
                  }
...
function postStart(client, mongoData, twitchData) {
  ...
  const str = `:movie_camera:**${name}** has just started streaming on Twitch.tv!` +
    `\n:video_game:Playing: **${twitchData.game}**\n<${twitchData.channel.url}>`;
...
function postChange(client, mongoData, twitchData) {
  logger.debug(`Attempting to post game change to ${mongoData.channels.length} channel(s)`);
  const name = twitchData.channel.display_name || twitchData.channel.name;
  const str = `:movie_camera:**${name}** has just changed games on Twitch.tv!` +
    `\n:video_game:Playing: **${twitchData.game}**\n<${twitchData.channel.url}>`;
  mongoData.channels.map(c => client.channels.get(c.channel_id))
    .filter(c => c)
    .forEach(c => {
