...
async function post(client, mongoData, stream, oldGame) {
  ...
  mongoData.channels.map(c => client.channels.get(c.channel_id))
    ...
    .forEach(c => {
      ...
      if (!c.guild.member(client.user).hasPermission('SEND_MESSAGES')) {
        utils.ownerError('Twitch Client', client,
          `Not allowed to post messages in guild **${c.guild.name}** channel **${c.name}** ` +
      ...
      c.sendEmbed(embed)
        .catch(err => {
          utils.ownerError('Twitch Client', client,
          `Twitch Client SendEmbed Error in guild **${c.guild.name}** channel **${c.name}** ` +
          `owned by **${c.guild.owner.user.username}**`);
          utils.ownerError('Twitch Client', client, err);
        });
    });
