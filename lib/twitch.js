...
async function checkLive(client) {
  ...
  if (client.twitch.channels === 0) {
    logger.debug('No available channels to send twitch streams posts to');
    return;
  }
    ...
    .then(async twitchResponses => {
      ...
      if (lastState) {
        ...
        gameChanged.forEach(stream => {
          ...
          post(client, results, stream, lastState.find(x => x.id === stream.id));
        });
      }
    ...
    Promise.all(promiseArray)
      .then(twitchResponses => {
        ...
        mergedResults.forEach(game => {
          game.expires = new Date().valueOf() + (1000 * 60 * 60 * 24 * 7);
...
async function post(client, results, stream, last) {
  ...
  const embed = new client.Discord.MessageEmbed()
    ...
    .setFooter('twitch.tv', 'https://blog.roblox.com/wp-content/uploads/2016/12/Twitch-Icon.png')
    .setTimestamp();
  if (last) {
    embed.addField('New Game:', stream.game_name || stream.game_id, true)
      .addField('Old Game:', last.game_name || last.game_id, true);
  } else {
    embed.addField('Game:', stream.game_name || stream.game_id);
  }
