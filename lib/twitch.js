...
module.exports = function TwitchLive(client) {
  ...
  function getUserData(loginOrId) {
    return new Promise((resolve, reject) => {
      ...
      request
        .get(client.utils.buildUri('https://api.twitch.tv/helix/users', query))
        .set({
  ...
  // Get user info from twitch api via login name or id
  function getKrakenUserData(id) {
    return new Promise((resolve, reject) => {
      // Query the Twitch API
      request
        .get(`https://api.twitch.tv/kraken/users/${id}`)
        .set({
          'Client-ID': config.twitch.client_id,
        ...
        .then((response) => {
          resolve(client.utils.get(['body'], response))
        })
        .catch(reject)
    })
  }
...
async function checkLive(client) {
  ...
  if (!results) {
    client.utils.ownerError(
      'Twitch Client',
      client,
      'Error getting Twitch channel results from DB',
    )
    return
  }
  ...
  const ids = results
    .filter((r) => {
      for (let i = 0; i < r.channels.length; i++) {
        const channel = client.channels.cache.get(r.channels[i].channel_id)
        if (channel) {
  ...
  client.twitch.channels = results
    .filter((r) => ids.indexOf(r.twitch_id) !== -1)
    .reduce((a, b) => {
  ...
  const promiseArray = idsArray.map((i) => {
    const uri = `https://api.twitch.tv/helix/streams?type=live${i
      .map((id) => `&user_id=${id}`)
      .join('')}`
    debug(uri)
    return request.get(uri).set({
      'Client-ID': config.twitch.client_id,
      Authorization: `Bearer ${client.config.twitch.access_token}`,
    })
  })
  ...
  Promise.all(promiseArray)
    .then(async (twitchResponses) => {
      ...
      const twitchResults = twitchResponses.map(
        (r) => client.utils.get(['body', 'data'], r) || [],
      )
      ...
      if (lastState) {
        ...
        newStreams.forEach((stream) => {
          if (
            ...
            games.get(stream.game_id).expires < new Date().valueOf()
          ) {
...
function resolveGames(client, gameIds) {
  return new Promise((resolve, reject) => {
    ...
    const promiseArray = idsArray.map((i) => {
      const uri = `https://api.twitch.tv/helix/games${i
        .map((id) => `&id=${id}`)
        .join('')
        .replace('&', '?')}`
      debug(uri)
...
async function post(client, results, stream, last) {
  ...
  const embed = new client.Discord.MessageEmbed()
    ...
    .addField(
      'Channel:',
      `[${mongoData.display_name}](https://www.twitch.tv/${mongoData.login})`,
      true,
    )
  ...
  if (last) {
    embed
      .addField('New Game:', stream.game_name || stream.game_id || 'NONE', true)
  ...
  mongoData.channels
    ...
    .forEach((c) => {
      ...
      c.send({ embed }).catch((err) => {
        client.utils.ownerError(
          `Twitch Client - Send to Channel (${c.id}) `,
          client,
          err,
        )
      })
...
async function updateUserData(client) {
  ...
  const promiseArray = idsArray.map((i) => {
    const uri = `https://api.twitch.tv/helix/users?id=${i.shift()}${i
      .map((id) => `&id=${id}`)
      .join('')}`
    debug(uri)
    return request.get(uri).set({
