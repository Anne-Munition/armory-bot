...
module.exports = function TwitchMessages(client) {
  ...
  streamInfoSocket.on('sub_games_advance_queue', () => {
    subGames().moveMembers(client, true)
      .then(() => {
        streamInfoSocket.emit('sub_games_advance_queue_response', true);
      })
      .catch(() => {
        streamInfoSocket.emit('sub_games_advance_queue_response', false);
      });
  });
};
