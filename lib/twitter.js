  ...
  function start(client) {
    ...
    client.mongo.twitterChannels.find({})
      .then(results => {
        ...
        logger.debug(ids);
        client.twitter.accounts = ids.length;
        client.twitter.channels = results.reduce((a, b) => a + b.channels.length, 0);
        ...
        .then(result => {
          if (result) {
            ...
            Promise.all(channelSendArray)
              .then(messageIds => {
                logger.debug('DONE messageIds', messageIds);
                const msgArray = messageIds.reduce((a, b) => a.concat(b), []);
                return saveTweetMessageIds(client, msgArray, tweet.id_str);
              })
              .then(() => {
                logger.debug('TWEET CHANNEL IDS SAVED');
...
function channelSend(channel, str, media) {
  return new Promise((resolve, reject) => {
    if (media.length > 0) {
      ...
      messageArray.push(sendFile(channel, media[0], str));
...
function saveTweetMessageIds(client, ids, tweetId) {
  return new Promise((resolve, reject) => {
    const tweetMessages = {
      tweet_id: tweetId,
      messages: ids,
