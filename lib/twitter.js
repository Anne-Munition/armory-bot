            ...
            if (!err && res.statusCode == 200) {//This GET request requires auth. This is our credentials and availability check
                ...
                twitter.stream('user', {with: 'user'}, (stream) => { //If connect is ok, startup stream listener
                    ...
                    stream.on('data', (tweet) => { //New tweet data received
                        ...
                        if (tweet.in_reply_to_user_id != null) return; //Exit if message is an @ to another user. 'PM'
    ...
    function handleTweet(tweet) {
        ...
        mongo.twitterChannels.find({}, (err, channels) => { //Get the channel ids to post tweets to
            ...
            getMedia(() => { //After all media is ready
                ...
                for (let i = 0; i < channels.length; i++) { //Post to all channels in tweets list
                    let channel = discord.client.channels.find('id', channels[i].channel_id);
                    channel.sendMessage(str)
                        .then((message) => {
                            msg_ids.push({channel: message.channel.id, message: message.id}); //Save header message id to list
                            postMedia(channel, () => { //Post all media in media list to each channel
                                next();
                            })
                                .catch(() => {
                                    next();
        ...
        function postMedia(channel, callback) {
            ...
            for (let i = 0; i < mediaUrls.length; i++) {
                ...
                channel.sendFile(file, tweet.id + ext)
                    .then((message) => { //Post media element
                        msg_ids.push({channel: message.channel.id, message: message.id}); //Save media message id to list
                        next();
                    })
                    .catch(() => {
                        next();
