...
module.exports = function (utils, options, discord, request, fs, path, mongo) {
                ...
                twitter.stream('user', {with: 'user'}, (stream) => { //If connect is ok, startup stream listener
                    ...
                    stream.on('data', (tweet) => { //New tweet data received
                        ...
                        if (tweet.delete) {
                            deleteTweet(tweet);
                            return;
                        } //Exit if activity is a deletion
    ...
    function handleTweet(tweet) {
            ...
            getMedia(() => {
                ...
                function next() {
                    if (count++ >= channels.length - 1) {
                        let entry = new mongo.tweetMessagesModel({
                            tweet_id: tweet.id_str,
                            messages: msg_ids
                        });
                        entry.save();
                        removeDir(path.join(__dirname, "../temp/" + tweet.id_str));
                    }
                }

                for (let i = 0; i < channels.length; i++) {
                    let id = channels[i].channel_id;
                    discord.quickMessage(id, str, (err, result) => {
                        msg_ids.push({channel: result.channel.id, message: result.id});
                        postMedia(id, () => {
                            next();
                        });
                    });
        ...
        function postMedia(channel, callback) {
            ...
            for (let i = 0; i < mediaUrls.length; i++) {
                ...
                discord.quickFile(channel, file, tweet.id + ext, (err, result) => {
                    msg_ids.push({channel: result.channel.id, message: result.id});
                    next();
                });
            }
        ...
        function createFrames(data) {
            return new Promise((resolve, reject) => {
                ...
                let child = exec("/usr/local/bin/ffmpeg -i \"" + data.mp4 + "\" -vf scale=250:-1:flags=lanczos,fps=10 \"" + frames + "\"",
                    (err) => {
                        if (err) {
                            reject(err);
                        } else {
        ...
        function createGIF(data) {
            return new Promise((resolve, reject) => {
                ...
                let child = exec("/usr/local/bin/gm convert -loop 0 \"" + frames + "\" \"" + gif + "\"",
                    (err) => {
                        if (err) {
                            reject(err);
                        } else {
    ...
    function deleteTweet(tweet) {
        mongo.tweetMessagesModel.findOne({tweet_id: tweet.delete.status.id_str}, (err, result) => {
