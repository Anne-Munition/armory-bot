  ...
  function start(client) {
    ...
    client.mongo.twitterChannels.find({})
      .then(results => {
        ...
        logger.debug('Streaming Twitter ids:', ids);
  ...
  return new Promise((resolve, reject) => {
    if (media.length > 0) {
      ...
      // Shift out the first element
       // TODO: Need to wait for first message to post before posting the rest of the media messages
      ...
      Promise.all(messageArray)
        .then(ids => {
          resolve({ channel: channel.id, messages: ids });
        })
        .catch(reject);
    } else {
