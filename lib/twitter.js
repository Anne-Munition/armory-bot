      ...
      .then(results => {
        ...
        twitter.on('connection aborted', () => {
          ...
          if (client.twitter.started === true) {
            client.utils.ownerError('Twitter Client', client, null, null, 'connection aborted - Restarting...');
            reset();
          }
        });
        ...
        twitter.on('connection error network', err => {
          ...
          client.utils.ownerError('Twitter Client', client, err, null, 'connection error network');
        });

        twitter.on('connection error stall', () => {
          logger.warn('Twitter Client - connection error stall');
          client.utils.ownerError('Twitter Client', client, null, null, 'connection error stall');
        });

        twitter.on('connection error http', httpStatusCode => {
          logger.warn('Twitter Client - connection error http', httpStatusCode);
          client.utils.ownerError('Twitter Client', client, httpStatusCode, null, 'connection error http');
        });

        twitter.on('connection rate limit', httpStatusCode => {
          logger.warn('Twitter Client - connection rate limit', httpStatusCode);
          client.utils.ownerError('Twitter Client', client, httpStatusCode, null, 'connection rate limit');
        });

        twitter.on('connection error unknown', err => {
          logger.warn('Twitter Client - connection error unknown', err);
          if (client.twitter.started === true) {
            client.utils.ownerError('Twitter Client', client, err, null, 'connection error unknown - Restarting...');
            reset();
          }
        });
        ...
        twitter.on('data error', err => {
          ...
          client.utils.ownerError('Twitter Client', client, err, null, 'data error');
        });
