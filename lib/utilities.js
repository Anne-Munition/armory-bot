...
module.exports = {
  ...
  getTwitchUserData: data => new Promise((resolve, reject) => {
    const query = {};
    if (/^\\d*$/.test(data)) {
      query.id = data;
    } else {
    ...
    const uri = module.exports.buildUri('https://api.twitch.tv/helix/users', query);
    logger.debug(uri);
    request.get(uri)
      .set({ 'Client-ID': config.twitch.client_id })
      .then(response => {
        if (!response.body || !response.body.data) return reject();
