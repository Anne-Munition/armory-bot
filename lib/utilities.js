...
module.exports = {
  ...
  formatDuration: (time) => {
    let str = '';
    const years = time._data.years;
    if (years > 0) {
      str += `${years} year${years === 1 ? '' : 's'} `;
      time = time.subtract(moment.duration(years, 'years'));
    }
    const months = time._data.months;
    if (months > 0) {
      str += `${months} month${months === 1 ? '' : 's'} `;
      time = time.subtract(moment.duration(months, 'months'));
    }
    const days = time._data.days;
    if (days > 0) {
      str += `${days} day${days === 1 ? '' : 's'} `;
    }
    str += `${time._data.hours} hour${time._data.hours === 1 ? '' : 's'} `;
    str += `${time._data.minutes} minute${time._data.minutes === 1 ? '' : 's'} `;
    str += `${time._data.seconds} second${time._data.seconds === 1 ? '' : 's'} `;
    return str.trim();
  },
