...
module.exports = {
  ...
  removeCommand: (client, cmdName) => {
    ...
    delete require.cache[
      require.resolve(path.join(client.commandsDir, `${cmdName}.js`))
    ]
      ...
      fs.readdir(client.commandsDir, (err, files) => {
        if (err) {
          ...
          const promiseArray = files.map((file) =>
            module.exports.loadOneCommand(client, file),
          )
          client.commands
            .map((c) => c.name)
  ...
  palette: (image) =>
    new Promise(async (resolve, reject) => {
      if (module.exports.isURI(image))
        image = (await request.get(image, { encoding: null })).body
      Vibrant.from(image).getPalette((err, palette) => {
        if (err) {
          reject(err)
        }
        const lightVibrant = module.exports.get(
          ['LightVibrant', '_rgb'],
          palette,
        )
  ...
  formatTimeDiff: (time) =>
    module.exports.formatDuration(moment.duration(moment().diff(moment(time)))),
  ...
  formatDuration: (time) => {
    ...
    str += `${time._data.hours} hour${
      parseInt(time._data.hours) === 1 ? '' : 's'
    } `
    str += `${time._data.minutes} minute${
      parseInt(time._data.minutes) === 1 ? '' : 's'
    } `
    str += `${time._data.seconds} second${
      parseInt(time._data.seconds) === 1 ? '' : 's'
    } `
    return str.trim()
  },
  ...
  usage: (msg, info) =>
    new Promise((resolve, reject) => {
      if (
        msg.channel.type !== 'dm' &&
        !msg.channel.permissionsFor(msg.client.user).has('SEND_MESSAGES')
      ) {
  ...
  dmDenied: (msg) =>
    new Promise((resolve, reject) => {
      msg
        .reply(`Unable to run command **${msg.cmd}** from a DM channel.`)
        .then(resolve)
        .catch(reject)
    }),
  ...
  ownerError: (type, client, err, msg, cmd, str) => {
    ...
    if (
      config.receiveErrors !== 'force' &&
      (!config.receiveErrors || client.user.id === '120105547633524736')
    )
      return
    ...
    if (owner) {
      ...
      if (msg) {
        ...
        details += `Guild: ${
          msg.channel.type === 'dm' ? 'DM' : guild.name || guild.username
        } (${guild.id})\n`
        details += `Channel: ${
      ...
      if (cmd) {
        ...
        if (client.commands.has(cmd)) {
          details += `Perms_Needed: ${JSON.stringify(
            client.commands.get(cmd).info.permissions,
          )}\n`
        }
      }
      if (msg && typeof msg !== 'string') details += `Content: ${msg.content}\n`
      details += '```'
      let errStr = ''
      if (err) {
        errStr = '```js\n'
      ...
      owner
        .send(
          `${type} Error on \`\`${moment().utc().format()}\`\`\n${
            msg ? details : ''
          }${errStr}`,
          { split: true },
        )
