...
module.exports = {
  ...
  colorGreen: () => module.exports.hexToInt('00ba25'),

  colorRed: () => module.exports.hexToInt('bb3631'),

  makePossessive: (name) => `${name}'${name.endsWith('s') ? '' : 's'}`,

  removeDiscordFormatting: (str) => {
    str = str.replace(/_/g, l => `${l}\u200b`);
    str = str.replace(/\*/g, l => `${l}\u200b`);
    str = str.replace(/`/g, l => `${l}\u200b`);
    return str;
  },

  ownerError: (client, err, msg, cmd) => {
    if (!config.receiveErrors) return;
    ...
    if (owner) {
      ...
      owner.sendMessage(`${msg ? details : ''}${errStr}`);
    }
  },
